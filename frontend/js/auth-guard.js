 ...
 *   AuthGuard.requireUser();    // any logged-in user
...
const AuthGuard = (() => {
  ...
  const requireUser = () => {
    ...
    // Admins who somehow land on user-dashboard get sent to admin panel
    if (user.role === 'admin') {
  ...
  return { requireAdmin, requireUser, requireGuest, redirectIfLoggedIn: requireGuest, logout };
})();
